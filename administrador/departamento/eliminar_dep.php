...
<html>
    ...
    <body>
       ...
       <?php
            ...
            $sql1="SELECT * FROM departamento WHERE Id_Departamento='$ccarp';";
            $res1=  mysqli_query($conex, $sql1);
            $lel=   mysqli_num_rows($res1);
            
            if ($lel>0) {
                $sql= "DELETE FROM departamento WHERE Id_Departamento='$ccarp';";
                 $qry= mysqli_query($conex, $sql);
            
            if ($qry==TRUE) {       
                    header("refresh:0;url=mostrar_departamento.php");
                    echo "<script> alert('El departamento ha sido eliminado correctamente! ');</script>";
